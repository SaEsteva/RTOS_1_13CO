...
const t_key_config  keys_config[] = { [TEC1_INDEX]= {TEC1},[TEC2_INDEX]= {TEC2} } ;
...
{
    BaseType_t signaled = xSemaphoreTake( keys_data[index].pressed_buton, 0 );
    if ( signaled == pdPASS )
    {
        return 1;
    }
...
int key_released( uint32_t index )
{
    BaseType_t signaled = xSemaphoreTake( keys_data[index].release_buton, 0 );
    if ( signaled == pdPASS )
    {
        return 1;
    }
...
void keys_init( void )
{
    ...
    for ( uint32_t i = 0 ; i < KEY_COUNT ; i++ )
    {
        ...
        keys_data[i].release_buton = xSemaphoreCreateBinary();
        keys_data[i].pressed_buton = xSemaphoreCreateBinary();

        configASSERT( keys_data[i].isr_signal != NULL );
        configASSERT( keys_data[i].release_buton != NULL );
        configASSERT( keys_data[i].pressed_buton != NULL );
    //}
...
static void keys_event_handler_button_pressed( uint32_t index )
{
    ...
    taskEXIT_CRITICAL();
    xSemaphoreGive( keys_data[index].pressed_buton ) ;
}
...
static void keys_event_handler_button_release( uint32_t index )
{
    ...
    xSemaphoreGive( keys_data[index].release_buton ) ;
}
