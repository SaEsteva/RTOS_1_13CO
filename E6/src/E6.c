...
#define WELCOME_MSG  "Ejercicio E_6.\r\n"
...
void tarea_led( void* taskParmPtr )
{
    ...
    while( TRUE )
    {
        if (xQueueReceive( queue_tec_pulsada, &dif,  0 ) == pdTRUE){// Espera el evento de cola 1
        	datos.led = leds_t[0];
        }else{
        	datos.led = leds_t[1];
        }
        ...
        gpioWrite( datos.led, ON );
        vTaskDelay( dif );

        datos.evento = blink;
        xQueueSend( queue_print, &datos,  portMAX_DELAY );

        gpioWrite( datos.led, OFF );
    ...
    while( TRUE )
    {
        ...
        switch(datos.evento){
        	...
        	default:
            	//xSemaphoreTake( mutex, portMAX_DELAY );			//abrir seccion critica
        		taskENTER_CRITICAL();
        		printf( "Error, evento incorrecto");
        		taskEXIT_CRITICAL();
        		//xSemaphoreGive( mutex ); //cerrar seccion critica
        		break;
        }
